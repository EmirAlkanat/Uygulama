...
export default function CalendarScreen() {
  ...
  const [selectedDayAnim] = useState(new Animated.Value(1));
  ...
  // Kartlar fade-in ve büyüme animasyonu
  useEffect(() => {
  ...
  const addPeriod = async () => {
    if (selectedStartDate) {
      ...
      // --- Otomatik yumurtlama hatırlatıcısı ekle ---
      const savedCycle = await AsyncStorage.getItem('CYCLE_LENGTH');
      const cycle = savedCycle ? Number(savedCycle) : (cycleLength || 28);
      const ovulationDate = new Date(selectedStartDate);
      ovulationDate.setDate(ovulationDate.getDate() + Math.floor(cycle / 2));
      const ovulationStr = toYYYYMMDD(ovulationDate);
      // REMINDERS'ı oku
      let reminders = [];
      try {
      ...
      // Aynı gün için zaten ovulation hatırlatıcısı var mı?
      const exists = reminders.some(r => r.type === 'ovulation' && r.time === '09:00' && r.date === ovulationStr);
      if (!exists) {
        reminders.push({
          type: 'ovulation',
          time: '09:00',
          description: 'Yumurtlama günü!',
          active: true,
          date: ovulationStr
        });
  ...
  // Takvimde gün seçilince zıplama
  const handleDayPress = (day) => {
    setSelectedStartDate(day.dateString);
    Animated.sequence([
      Animated.timing(selectedDayAnim, { toValue: 1.2, duration: 120, useNativeDriver: true }),
      Animated.spring(selectedDayAnim, { toValue: 1, friction: 3, useNativeDriver: true })
    ]).start();
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <LinearGradient colors={['#232046', '#3B2B5A']} style={{ flex: 1 }}>
        <Animated.ScrollView style={{ opacity: cardAnim, transform: [{ scale: cardAnim.interpolate({inputRange:[0,1],outputRange:[0.95,1]}) }] }} contentContainerStyle={{paddingBottom: 40}} showsVerticalScrollIndicator={false}>
          {/* Başlık */}
          ...
          <SafeAreaView style={{ flex: 1 }}>
            ...
            <ScrollView ref={scrollRef} style={styles.scrollView} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom: 60}}>
              <View style={styles.headerGradient}>
                <Animated.View style={[styles.header, { opacity: fadeAnim, transform: [{ translateY: slideAnim }], marginTop:60 }]}> 
                  <View style={styles.headerContent}>
                    <View style={styles.logoContainer}>
                      <Image source={require('./assets/adetresim.png')} style={{ width: 60, height: 60, resizeMode: 'contain'}} />
                    </View>
                    <Text style={[styles.appTitle, { color: '#B83280',marginTop:10 }]}>Ishe</Text>
